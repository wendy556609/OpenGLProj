...
#define GRID_SIZE 100 // must be an even number
...
void init(void)
{
	...
	_pFloor = new Flat('M', GRID_SIZE, vT, 0, vec3(0, 1.0f, 0));

	vT.x = 0; vT.y = 50.0f; vT.z = 0;
	_pTop = new Flat('T', GRID_SIZE, vT, 0, vec3(0, -1.0f, 0));

	vT.x = -50.0f; vT.y = 25.0f; vT.z = 0;
	_LeftWall = new Flat('L', GRID_SIZE, vT, 90, vec3(1.0f, 0, 0));

	vT.x = 50.0f; vT.y = 25.0f; vT.z = 0;
	_RightWall = new Flat('R', GRID_SIZE, vT, 90, vec3(1.0f, 0, 0));

	vT.x = 0.0f; vT.y = 25.0f; vT.z = 50.0f;
	_FrontWall = new Flat('F', GRID_SIZE, vT, 90, vec3(0, 0, -1.0f));

	vT.x = 0.0f; vT.y = 25.0f; vT.z = -50.0f;
	_BackWall = new Flat('B', GRID_SIZE, vT, 90, vec3(0, 0, -1.0f));
...
void onFrameMove(float delta)
{
	...
	_pFloor->Update(g_Light1, delta);
	_pTop->Update(g_Light1, delta);
	_LeftWall->Update(g_Light1, delta);
	_RightWall->Update(g_Light1, delta);
	_FrontWall->Update(g_Light1, delta);
	_BackWall->Update(g_Light1, delta);
	//Model
	_pTeaPot->Update(g_Light1, delta);
	_pTable->Update(g_Light1, delta);
	_pBench[0]->Update(g_Light1, delta);
	_pBench[1]->Update(g_Light1, delta);
	_pBench[2]->Update(g_Light1, delta);
